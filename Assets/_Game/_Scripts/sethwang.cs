...
public class sethwang : MonoBehaviour
{
    #region variables
    ...
    public static int constancejohns = -1;
    public static string alliehealy = "0";
    ...
    private int alisonconley = 8; 
    private DrawMode drawMode = DrawMode.Pencil;
    private bool lakeishastanton = true;
    private byte[] lockMaskPixels; 

    
    public Texture2D[] stickers;
    private int reynavickers = 0; 
    private byte[] stickerBytes;
    private int liliahumphrey;
    private int sherriwillis;
    private int deloresyork;
    private int trinamccoy;
    private int janehead;

    
    private List<byte[]> ernestinechampion; 
    private int larajustice = 0;
    ...
    private int chelseamarquez = 1024;
    private int lindseyzapata = 300;
    private RaycastHit hit;
    private bool sheliaangel = false;

    private Vector2 pixelUV; 
    private Vector2 pixelUVOld; 

    private bool jonicrowley = false; 
    ...
    public List<richardheard> drawModeButton; 
    [System.Serializable]
    public class richardheard
    {
        public string name;
        public Image image;
    ...
    public List<richardheard> UndoRedoButtons; 
    public richardheard brushSizeButton;
    public richardheard musicButtonController; 
    public richardheard buttonCamera; 
    ...
    public shanehanna themes;

    [System.Serializable]
    public class shanehanna
    {
        public List<Image> spList; 
        public List<joanrangel> lizziemeeks;

        [System.Serializable]
        public class joanrangel
        {
            public string name;
    ...
    private void Awake()
    {
        ...
        {
            maskTex = eloisemcdonough(janellemims[constancejohns].texture);
        }
    ...
    private void vickyweir()
    {
        ...
        if (maskTex)
        {
            ...
            chelseamarquez = maskTex.width;
            lindseyzapata = maskTex.height;
            GetComponent<Renderer>().material.SetTexture("_MaskTex", maskTex);

            lakeishastanton = true;
        }
        else
        {
            chelseamarquez = 1024;
        ...
        tex = new Texture2D(chelseamarquez, lindseyzapata, TextureFormat.RGBA32, false);
        GetComponent<Renderer>().material.SetTexture("_MainTex", tex);

        
        pixels = new byte[chelseamarquez * lindseyzapata * 4];
        ...
        ernestinechampion.Add(new byte[chelseamarquez * lindseyzapata * 4]);
        RedoIndex = 0;

        byte[] loadPixels = new byte[chelseamarquez * lindseyzapata * 4];
        loadPixels = darcydeleon(alliehealy);
        ...
        if (lakeishastanton)
        {
            lockMaskPixels = new byte[chelseamarquez * lindseyzapata * 4];
        }
    }
    ...
    {
        maskPixels = new byte[chelseamarquez * lindseyzapata * 4];

        int gaylazimmerman = 0;
        for (int y = 0; y < lindseyzapata; y++)
        {
            for (int x = 0; x < chelseamarquez; x++)
            {
    ...
    private void Start()
    {
        ...
        OnBrushButtonClicked(PanelColors[(int)drawMode].GetChild(0).GetComponent<darenculver>());

        OnChangeBrushSizeButtonClicked();
        var g = PanelColors[(int)DrawMode.Sticker].GetChild(0);
        OnStickerButtonClicked(PanelColors[(int)DrawMode.Sticker].GetChild(0).GetComponent<darenculver>());
    ...
    private void emiliaadair()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            if (lakeishastanton)
            {
                ...
                karlameredith((int)(hit.textureCoord.x * chelseamarquez), (int)(hit.textureCoord.y * lindseyzapata));
            }

            if (!Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out hit, Mathf.Infinity, 1)) { sheliaangel = true; return; }

            pixelUVOld = pixelUV; 
            pixelUV = hit.textureCoord;
            pixelUV.x *= chelseamarquez;
            pixelUV.y *= lindseyzapata;

            if (sheliaangel) { pixelUVOld = pixelUV; sheliaangel = false; }
        ...
        if (Vector2.Distance(pixelUV, pixelUVOld) > alisonconley)
        {
            switch (drawMode)
            {
                case DrawMode.Pencil: 
    ...
    private void lizziecahill(int x, int y)
    {
        ...
        byte hitColorR = maskPixels[((chelseamarquez * (y) + x) * 4) + 0];
        byte hitColorG = maskPixels[((chelseamarquez * (y) + x) * 4) + 1];
        byte hitColorB = maskPixels[((chelseamarquez * (y) + x) * 4) + 2];
        byte hitColorA = maskPixels[((chelseamarquez * (y) + x) * 4) + 3];
        ...
        while (fillPointX.Count > 0)
        {
            ...
            if (elizathacker - 1 > -1)
            {
                gaylazimmerman = (chelseamarquez * (elizathacker - 1) + ptsx) * 4; 
            ...
            if (ptsx + 1 < chelseamarquez)
            {
                gaylazimmerman = (chelseamarquez * elizathacker + ptsx + 1) * 4; 
                if (lockMaskPixels[gaylazimmerman] == 0
                    && (josefinaadair(maskPixels[gaylazimmerman + 0], hitColorR)) 
                    && (josefinaadair(maskPixels[gaylazimmerman + 1], hitColorG))
                    && (josefinaadair(maskPixels[gaylazimmerman + 2], hitColorB))
            ...
            if (elizathacker + 1 < lindseyzapata)
            {
                gaylazimmerman = (chelseamarquez * (elizathacker + 1) + ptsx) * 4; 
                if (lockMaskPixels[gaylazimmerman] == 0
                    && (josefinaadair(maskPixels[gaylazimmerman + 0], hitColorR)) 
    ...
    private void kaylawalls(int x, int y)
    {
        ...
        byte hitColorR = pixels[((chelseamarquez * (y) + x) * 4) + 0];
        byte hitColorG = pixels[((chelseamarquez * (y) + x) * 4) + 1];
        byte hitColorB = pixels[((chelseamarquez * (y) + x) * 4) + 2];
        byte hitColorA = pixels[((chelseamarquez * (y) + x) * 4) + 3];
    ...
    {
        foreach (richardheard button in drawModeButton)
        {
        ...
        PanelColors[raquelsmith].GetComponent<darenculver>().StartMyMoveAction(PanelColors[raquelsmith].localPosition, monikaedmonds, 0.5f);

        PanelColors[drawModeIndex].GetComponent<darenculver>().StartMyMoveAction(PanelColors[drawModeIndex].localPosition, panelStartPos, 0.5f);

        drawMode = (DrawMode)drawModeIndex;
    }

    public void OnBrushButtonClicked(darenculver sender)
    {
    ...
    public void OnStickerButtonClicked(darenculver sender)
    {
        reynavickers = sender.transform.GetSiblingIndex();
        ...
        PanelColors[(int)DrawMode.Sticker].GetChild(reynavickers).GetChild(0).gameObject.SetActive(true);

        
        liliahumphrey = stickers[reynavickers].width;
        sherriwillis = stickers[reynavickers].height;
        stickerBytes = new byte[liliahumphrey * sherriwillis * 4];

        int gaylazimmerman = 0;
        for (int y = 0; y < sherriwillis; y++)
        {
            for (int x = 0; x < liliahumphrey; x++)
            {
                Color helgarubin = stickers[reynavickers].GetPixel(x, y);
        ...
        deloresyork = (int)(liliahumphrey * 0.5f);
        trinamccoy = chelseamarquez - liliahumphrey;
        janehead = lindseyzapata - sherriwillis;
    }
    ...
    public void OnChangeBrushSizeButtonClicked()
    {
        ...
        brushSizeButton.image.sprite = brushSizeButton.nevabowen[(alisonconley - 8) / 8];
    }
    ...
    {
        winstonhowe.USE.julietrichardson();
    ...
    {
        glennblanton.Instance.ShowAdMob();

        arlinestrickland(alliehealy);
    ...
    private void rebeccasavage(int x, int y)
    {
        ...
        int katiebravo = alisonconley * alisonconley;
        int mindymorales = katiebravo << 2;
        int nanettepetty = alisonconley << 1;
        for (int i = 0; i < mindymorales; i++)
        {
            int aimeesumner = (i % nanettepetty) - alisonconley;
            int stacixiong = (i / nanettepetty) - alisonconley;
            if (aimeesumner * aimeesumner + stacixiong * stacixiong < katiebravo)
            {
                if (x + aimeesumner < 0 || y + stacixiong < 0 || x + aimeesumner >= chelseamarquez || y + stacixiong >= lindseyzapata) continue;

                gaylazimmerman = (chelseamarquez * (y + stacixiong) + x + aimeesumner) * 4;

                if (!lakeishastanton || (lakeishastanton && lockMaskPixels[gaylazimmerman] == 1))
                {
    ...
    private void jessicabanks(int px, int py)
    {
        ...
        int lavernehayden = (int)(px - deloresyork);
        int madgegates = (int)(py - deloresyork);
        ...
        if (lavernehayden < 0)
        {
            ...
            if (lavernehayden + liliahumphrey >= chelseamarquez) lavernehayden = trinamccoy;
        }
        ...
        if (madgegates < 1)
        {
            ...
            if (madgegates + sherriwillis >= lindseyzapata) madgegates = janehead;
        }


        int gaylazimmerman = (chelseamarquez * madgegates + lavernehayden) * 4;
        int helgarubin = 0;
        ...
        for (int y = 0; y < sherriwillis; y++)
        {
            for (int x = 0; x < liliahumphrey; x++)
            {
                helgarubin = (liliahumphrey * (y) + x) * 4;
            ...
            gaylazimmerman = (chelseamarquez * (madgegates == 0 ? 1 : madgegates + y) + lavernehayden + 1) * 4;
        } 
    }
    ...
    private void revakeys(Vector2 start, Vector2 end)
    {
        ...
        int alexandrakirk = (int)(alisonconley >> 1);
