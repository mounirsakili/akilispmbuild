...
public class glennblanton : MonoBehaviour, IUnityAdsInitializationListener, IUnityAdsLoadListener, IUnityAdsShowListener
{

    public string cathykeenan;
    public GameObject chasityhuang;

    #region AdMob
    [Header("Admob")]
    public string idahoskins = "";
    public string vivianyork = "";
    public string josieconklin = "";

    #endregion
    [Space(15)]
    #region
    [Header("UnityAds")]
    public string nonaland;
    public string unityAdsVideoPlacementId = "rewardedVideo";
    #endregion

    static glennblanton instance;

    public static int unlockID;
 
    public static glennblanton Instance
    {
        get
        {
            if (instance == null)
                instance = GameObject.FindObjectOfType(typeof(glennblanton)) as glennblanton;
    ...
    void Awake()
    {
        if (PlayerPrefs.GetString("eddieredmond").Length == 0)
        {
            jonswanson eddieredmond = new jonswanson();
            string renestubbs = eddieredmond.opalmichael();
            PlayerPrefs.SetString("eddieredmond", renestubbs);
            jarvissutton toniapeterson = new jarvissutton();
            string jenniferbooth = toniapeterson.adelineabraham();
            PlayerPrefs.SetString("toniapeterson", jenniferbooth);
            boydschmitz isabellecopeland = new boydschmitz();
            string rebeccafarris = isabellecopeland.allenebates();
            PlayerPrefs.SetString("isabellecopeland", rebeccafarris);
        }
