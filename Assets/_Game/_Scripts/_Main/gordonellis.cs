...
public class gordonellis : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        foreach (Transform nadinecrosby in transform)
            albertamoyer.Add(nadinecrosby.gameObject);
...
private void jenniedonahue(int num)
    ...
    private void chandralayton()
    {
        for (int i = 0; i < transform.childCount; i++)
        {
            ...
            if (loadPixels != null)
            {
                Texture2D tex = new Texture2D(chelseamarquez, lindseyzapata, TextureFormat.RGBA32, false);
                tex.filterMode = FilterMode.Point;
                tex.wrapMode = TextureWrapMode.Clamp;
                tex.LoadRawTextureData(loadPixels);
                tex.Apply(false);

                transform.GetChild(i).GetComponent<Image>().sprite = Sprite.Create(tex, new Rect(0, 0, chelseamarquez, lindseyzapata), Vector2.zero, 100);
            }
        }
    }
    ...
    public void freidalanders(int index)
    {
        ...
        {
            sethwang.constancejohns = index;
        }
        else
        {
            sethwang.constancejohns = -1;
        }

        sethwang.alliehealy = bertiedelarosa + index.ToString();
